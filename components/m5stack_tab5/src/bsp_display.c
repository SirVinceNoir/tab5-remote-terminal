...
#include "esp_lcd_st7121.h" /* LOCAL PATCH: see fw_version==1 branch below */
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
    ...
    const esp_lcd_dpi_panel_config_t dpi_config_st7123 = {
        ...
        .video_timing =
        {
            ...
            .hsync_front_porch = 40,
            /*
             * LOCAL PATCH (not upstream): our unit's ST712x touch controller
             * reports FW_VERSION_REG(0x0000) == 1, which per M5Stack's own
             * factory firmware (M5Tab5-UserDemo bsp_detect_display_type())
             * means this is actually the ST7121 timing sub-variant, not the
             * ST7123 timing this BSP always applies for any "board version 2"
             * unit. Using M5Stack's ST7121 branch values here instead.
             */
            .vsync_back_porch  = 24,
    ...
    /*
     * LOCAL PATCH (not upstream): our unit's ST712x touch controller reports
     * FW_VERSION_REG(0x0000) == 1 via I2C, which per M5Stack's own factory
     * firmware (M5Tab5-UserDemo bsp_detect_display_type()) means this needs
     * the ST7121 driver + timing, not the ST7123 driver this BSP always uses
     * for "board version 2". M5Stack calls a genuinely different panel
     * driver component (esp_lcd_st7121, not esp_lcd_st7123 with tweaked
     * config) for this case, with init_cmds=NULL (driver's own default table).
     */
    ...
    if (bsp_get_board_version() == 1) {
        ...
        /* LOCAL PATCH: use the ST7121 driver -- see vendor_config_st7121 above */
